...
export const getPlaceNames = async (
  ...
  try {
    ...
    const encodedMunicipality = municipalityPart
      ...
      : null;
    const res = await fetch(
      `${env.geoNorgeApiBaseUrl}/stedsnavn/v1/navn?sok=${encodedQuery}*${encodedMunicipality != null ? '&kommunenavn=' + encodedMunicipality + '*' : ''}&treffPerSide=15&side=${page}&fuzzy=true`,
    );
    if (!res.ok) {
